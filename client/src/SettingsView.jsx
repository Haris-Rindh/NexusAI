...
const SettingsView = ({ hasLinkedIn }) => {
  const { user } = useUser();
  ...
  const handleDisconnectLinkedIn = async () => {
    try {
      const acc = user?.externalAccounts?.find(a => a.provider === 'oauth_linkedin_oidc' || a.provider === 'oauth_linkedin');
      if (acc) {
        await acc.destroy();
    ...
    <div className="max-w-6xl mx-auto p-8 font-sans text-slate-900">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-3xl border border-slate-200 p-8 shadow-sm min-h-[600px]">
          ...
          {activeSection === 'connections' && (
            <div className="space-y-6 animate-in fade-in duration-500 max-w-2xl">
              <div>
                ...
                <p className="text-slate-500 mt-1 mb-6">Link your social media profiles to enable one-click publishing.</p>
              </div>

              <div className="border border-slate-200 rounded-2xl p-6 flex justify-between items-center bg-slate-50 relative overflow-hidden">
                {hasLinkedIn && <div className="absolute top-0 left-0 w-1 h-full bg-green-500"></div>}

                <div className="flex gap-4 items-center">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-inner ${hasLinkedIn ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-400'}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" /></svg>
                  </div>
                  <div>
                    <h3 className="font-bold text-lg text-slate-900">LinkedIn Profile</h3>
                    <p className="text-sm text-slate-500">{hasLinkedIn ? 'Connected correctly' : 'Not connected'}</p>
                  </div>
                </div>

                <div>
                  {hasLinkedIn ? (
                    <button onClick={handleDisconnectLinkedIn} className="px-5 py-2 border border-slate-300 text-slate-600 font-bold rounded-lg hover:bg-slate-100 transition text-sm">Disconnect</button>
                  ) : (
                    <button onClick={handleConnectLinkedIn} className="px-5 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition shadow-md text-sm">Connect Profile</button>
                  )}
                </div>
              </div>
            </div>
          ...
          {/* 4. NOTIFICATIONS & SECURITY (Placeholders) */}
