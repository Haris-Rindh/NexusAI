...
const CarouselStudio = ({ isGuestMode, onGenerateSuccess, hasLinkedIn }) => {
  const { user } = useUser();
  ...
  const handleAiGenerate = async () => {
    if (isGuestMode) return toast.error("Sign up to generate slides!");
  ...
  return (
    <div className="flex flex-col lg:flex-row h-auto gap-6 pb-20 font-sans">
      ...
      <div className="w-full lg:w-1/6 flex flex-col gap-3 order-3">
        ...
        <button onClick={() => handleSave('draft')} disabled={isSaving} className={`w-full ${hasLinkedIn ? 'py-2' : 'py-4 mt-8 bg-white border border-slate-200'} text-blue-600 font-bold rounded-xl hover:bg-blue-50 flex flex-col items-center justify-center gap-1`}>
          {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className={hasLinkedIn ? "w-4 h-4" : "w-6 h-6"} /><span className={hasLinkedIn ? "text-xs" : "text-sm"}>Save Draft</span></>}
        </button>
        <button onClick={downloadPDF} disabled={isDownloading} className={`w-full py-3 ${hasLinkedIn ? 'text-slate-500' : 'bg-slate-900 text-white shadow-lg'} font-bold rounded-xl flex items-center justify-center gap-2`}>
